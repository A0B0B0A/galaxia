...
finish_text = font2.render(f"PRESS F TO RESTART", True, (255, 255, 255))
while run:
    for e in event.get():
        if e.type == QUIT:
            run = False
            ...
            if e.key == K_ESCAPE:
                run = False
                sys.exit()
            if e.key == K_SPACE and not finish and not start_screen:
                player.fire()
            if finish:
                if e.key == K_f:
                    finish= False
    ...
    if start_screen:
        ...
        if finish:
            ...
            window.blit(finish_text, (WIDTH/2 - finish_text.get_width()/2,
                                       HEIGHT/2 - finish_text.get_height()/2 + 100))
